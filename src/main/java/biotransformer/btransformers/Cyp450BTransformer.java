...
public class Cyp450BTransformer extends Biotransformer {
	...
	public Cyp450BTransformer(BioSystemName bioSName, boolean useDB, boolean useSubstitution) throws Exception {
		super(bioSName, useDB, useSubstitution);		
		setCyp450EnzymesAndReactionList();
		if(useDB) {
			this.rfdb = new RetriveFromDB("hmdb", "cyp450", false, 30, useSubstitution);
		}
	...
	public ArrayList<Biotransformation> predictCyp450BiotransformationsByMode(IAtomContainer substrate, int mode, boolean preprocess, boolean filter, double threshold)  throws Exception{
		ArrayList<Biotransformation> biotransformations = new ArrayList<Biotransformation>();
		//BioTransformerAPI_cypreact cypReact = new BioTransformerAPI_cypreact();
		...
		List<String> substrateForEnzymes = cypReact.predictEnzymeSubstrateSpecificity(substrate, this.cyp450Enzymes);
		try{
			...
			System.out.println(this.smiGen.create(substrate));
			e.printStackTrace();
		}
		return biotransformations;
	}
	 ...
	 * @throws Exception
	 */
	public ArrayList<Biotransformation> predictCypProductBiotransformation(IAtomContainer substrate, List<String> enzymeList, boolean useCypReact) throws  Exception{
		ArrayList<Biotransformation> biotransformations = new ArrayList<Biotransformation>();
		...
		oneMole = Utilities.create3DCoordinates(oneMole);
...
//		}
		BioTransformerAPI_cyproduct biotransformer_cyprdocut = new BioTransformerAPI_cyproduct();

		
		//IAtomContainerSet cyProduct_results = biotransformer_cyprdocut.runOnePrediction(oneMole, new ArrayList<String> (enzymeList), false, 0.0);
		IAtomContainerSet cyProduct_results = null;
		...
		ArrayList<Biotransformation> cyProduct_extra_Biotransformations = convertMLProductToBioTransformation(substrates_temp, cyProduct_results);
	 ...
	 * @throws Exception
	 */
	public ArrayList<Biotransformation> predictByCyProductAndCyp450(IAtomContainer substrate, boolean preprocess, boolean filter, double threshold, List<String> enzymeList) throws Exception{
		ArrayList<Biotransformation> biotransformations = new ArrayList<Biotransformation>();
...
//		}
		...
		IAtomContainerSet removedNonsense = DefaultChemObjectBuilder.getInstance().newInstance(IAtomContainerSet.class);
		for(int i = 0; i < cyProduct_results.getAtomContainerCount(); i++){
			if(cyProduct_results.getAtomContainer(i).getAtomCount() > 1 && this.isValidMetabolte(cyProduct_results.getAtomContainer(i))){
				removedNonsense.addAtomContainer(cyProduct_results.getAtomContainer(i));
				String inChiKey = this.inchiGenFactory.getInChIGenerator(cyProduct_results.getAtomContainer(i)).getInchiKey();
				cyProduct_results.getAtomContainer(i).setProperty("InChIKey", inChiKey);
				cyProduct_results.getAtomContainer(i).setProperty("SMILES", this.smiGen.create(cyProduct_results.getAtomContainer(i)));
		...
		ArrayList<Biotransformation> cyProduct_extra_Biotransformations = convertMLProductToBioTransformation(substrates_temp, notIncluded_results);
		biotransformations.addAll(cyProduct_extra_Biotransformations);
		return Utilities.selectUniqueBiotransformations(biotransformations);
	}
...
//				}			
	...
	public ArrayList<Biotransformation> predictCyp450BiotransformationChainByMode(IAtomContainerSet substrates, boolean preprocess, boolean filter, int nrOfSteps, double threshold, int cyp450Mode) throws Exception{		
		...
		while(nrOfSteps>0){
			...
			//Add the useDB feature here
			ArrayList<Biotransformation> currentBiotransformations_fromBT = predictCyp450BiotransformationsByMode(containers, cyp450Mode, preprocess, filter, threshold);			
